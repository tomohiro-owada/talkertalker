...
func main() {
	// Create speaker for Japanese
	speaker, err := tts.NewSpeaker(tts.Japanese)
	if err != nil {
		log.Fatal(err)
	}

	// Create Fyne app with unique ID
	a := app.NewWithID("com.talkertalker.japanese")
	w := a.NewWindow("TalkerTalker - 日本語")
	...
	// Setup global hotkey (Ctrl+Shift+G)
...
func (a *AppJA) setupHotkey() {
	// Register Ctrl+Shift+G to activate window
	hook.Register(hook.KeyDown, []string{"ctrl", "shift", "g"}, func(e hook.Event) {
...
func (a *AppJA) speakText(text string) {
	if text == "" {
